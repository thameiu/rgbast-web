...
/** Whether a registration request is pending. */
const loading = ref(false);

/** Error message string to display. */
const errorMsg = ref('');

/** Success message string to display after registration. */
const successMsg = ref('');

const PASSWORD_REGEX = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[#?!@$%^&*\-.]).{8,255}$/;

/** Computed password strength score from 0–5. */
...
/** Tailwind color class for the strength bar segments. */
...
/**
 * Submits the registration form. Shows a success message, then emits navigate to 'login'.
 */
